...
`PRIMER.md` requires ongoing maintenance. The Argus Day 30 health check found 8 of 19 entities non-compliant; most failures were `passenger.json` gaps — Iris was the only PRIMER failure. Stale context is a real failure mode — an entity with an outdated PRIMER is operating on a false picture of its own situation. Inactive entities with outdated PRIMERs are dormant problems. Active entities with outdated PRIMERs are active ones.
