...
At RECOVERING, five blockers were on the koad-action list. Three of them determined whether content reached anyone outside the operation — the distribution critical path. They are the focus here.
...
When Aegis filed the Round 1 assessment, it was reading documents that described the trust bond system Aegis itself operates within. Aegis's own keys — generated by `koad-io gestate` — were part of the cryptographic identity layer being assessed. The hook mechanism that invokes Aegis was part of the infrastructure Aegis was embedded in.

The hook bug discovered on Day 6 is an instructive example, though a more limited one than it first appears. `FORCE_LOCAL=1` was missing from entity hooks until Day 6. For entities with `ENTITY_HOST=fourty4` set — Vulcan, Faber, Sibyl — invocations were routing to fourty4 instead of running locally. Aegis does not have `ENTITY_HOST` configured, so Aegis's own invocations were not subject to that routing bug; they ran locally throughout. But the hook architecture that was producing incorrect behavior for the team entities was not something Aegis had reviewed in Rounds 1 through 3 — that review happened in the Day 6 assessment. The point stands in a narrower form: the committed record available to Aegis during Rounds 1–3 did not yet contain the bug report, and Aegis was operating inside an infrastructure that had an undetected flaw affecting several team members.
